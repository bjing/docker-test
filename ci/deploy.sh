...
echo "$GENERATED" > ci/task-definition.json
cat ci/task-definition.json
...
docker run --rm anigeo/awscli      \
   ...
   --family $FAMILY                \
   --region $REGION                \
   --cli-input-json file://ci/task-definition.json | grep "revision" | cut -d ':' -f 2 | cut -c2- > revision.txt
